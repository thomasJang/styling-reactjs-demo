...
function tsConfig(config, env, babelPlugins) {
  ...
  // if (env === 'production') {
  //   if (!tsLoader.include) tsLoader.include = [];
  //   if (!Array.isArray(tsLoader.include)) tsLoader.include = [tsLoader.include];
  //   tsLoader.include.push(/node_modules/);
  // }
...
function jsConfig(config, env, babelPlugins) {
  ...
  if (env === 'production') {
    if (!jsLoader.include) jsLoader.include = [];
    if (!Array.isArray(jsLoader.include)) jsLoader.include = [jsLoader.include];
    jsLoader.include.push(/node_modules/);
  }
